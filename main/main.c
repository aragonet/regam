...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_READ_EVT:
    {
        ...
        char value[22];
        memcpy(value, wifiNames + param->read.offset, 22);
        value[22] = 0;
        ESP_LOGI(GATTS_TAG, "VALUE: %s, %d", value, strlen(value));
        int len = 22;
        if (strlen(value) < 21)
        {
            len = strlen(value);
        ...
        strncpy((char *)rsp.attr_value.value, value, 22);
    ...
    case ESP_GATTS_WRITE_EVT:
    {
        ...
        if (!param->write.is_prep)
        {
            ...
            myDataValue = calloc(param->write.len + 1, sizeof(char));
            ...
            ESP_LOGI(GATTS_TAG, "something: %s", param->write.value);
...
void rebootIfNeeded()
{
    ...
    else
    {
        printf("REBOOT\n");
        int rebootValue = 0;
        nvs_get_i8(my_handle, "reboot_value", &rebootValue);
        printf("REBOOT %d\n", rebootValue);
        if (rebootValue == 1){
            //Ignore error cause is going to restart anyway
            nvs_set_i8(my_handle, "reboot_value", 0);
